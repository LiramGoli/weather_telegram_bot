...
def send_welcome(message):
  weatherBot.reply_to(
    message,
    "על מנת להתחיל, רשום את המילה ערים ולאחר מכן הקש את מספר העיר המבוקשת על מנת לקבל מידע על מזג האוויר בה"
  )
...
def get_api_call(city):
  ...
  for i in range(len(hours)):
    if(int(hours[i].split("T")[1].split(":")[0])>=now.hour)\
      ...
      weather_dict["hourly_weather"].append({
        "hour":
        hours[i].split("T")[1].split(":")[0],
        "temp":
        temp_by_hours[i],
        "rain":
        rain_by_hours[i]
